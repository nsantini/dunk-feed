...
- **PRD phase**: 0
- **Size**: standard
- **Design**: docs/01-TECH-DESIGN.md §3, §4

## Outcome

After this ships, all four gates in AGENTS.md pass on the `upstage` crate:
...
|---|---|---|---|
| BC1 | `UPSTAGE_HOSTNAME` | missing | `config::load` returns `ConfigError::Missing("UPSTAGE_HOSTNAME")`; `main.rs` prints it and exits 1 |
| BC2 | `UPSTAGE_PUBLISHER_DID` | missing | Same path as BC1 |
| BC3 | `UPSTAGE_K` | malformed, e.g. `"five"` | `ConfigError::Invalid{name, value, reason}`, caught in `src/config.rs` |
| BC4 | `UPSTAGE_W_REPOST` | empty string | Treated as malformed; same error as BC3 |
| BC5 | `UPSTAGE_FEED_RKEY` | unset | Falls back to the default `"upstaged"`, no error |
...
| BC8 | Unknown environment variable, e.g. `UPSTAGE_TYPO` | extra, unrecognised | Ignored; not an error |
...
- [ ] AC2 — A missing `UPSTAGE_HOSTNAME` fails fast with a clear message. Checked by: `cargo test config::tests::missing_required_var_fails`
- [ ] AC3 — A malformed `UPSTAGE_K` fails fast with a clear message. Checked by: `cargo test config::tests::malformed_number_fails`
- [ ] AC4 — `.env.example` lists every variable from TECH-DESIGN §4. Checked by: reviewer reads `.env.example`
- [ ] AC5 — `upstage run` / `validate` / `publish` / `dump` each print their own name and exit 0. Checked by: `cargo test cli::tests::stub_subcommands_exit_zero`
...
- Log format: `tracing_subscriber` with an `EnvFilter` built from `UPSTAGE_LOG`,
...
- `UPSTAGE_DROP_LABELS` is parsed here as a comma-separated list into
