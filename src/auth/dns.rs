...
//!
//! Review round 3, defect AX: every special-purpose block is blocked in
//! full. The check fails closed. Round 2, defect AV had carved out the
//! IANA registries' "Globally Reachable: True" entries inside
//! `192.0.0.0/24` and `2001::/23` (`192.0.0.9/32`, `192.0.0.10/32`,
//! `2001:1::1/128` to `2001:1::3/128`, `2001:3::/32`, `2001:4:112::/48`,
//! `2001:20::/28`, `2001:30::/28`); this round removes every one of those
//! exceptions, so `192.0.0.0/24` and `2001::/23` are each blocked whole,
//! the same as every other range here. Defect AW's additions stay:
//! `100:0:0:1::/64` (a dummy prefix, not the whole `100::/64` discard-only
//! block above it) and `5f00::/16` (SRv6 SIDs) are not globally routable
//! and are blocked.
//!
...
/// round 3, defect AX: `192.0.0.0/24` is blocked in full — round 2's
/// `192.0.0.9/32` and `192.0.0.10/32` exceptions are removed, so the check
/// fails closed.
...
fn is_public_v6(ip: Ipv6Addr) -> bool {
    ...
    // the top 7 bits of the second segment are 0. Blocked whole (review
    // round 3, defect AX): the round 2 (defect AV) globally reachable
    // exceptions inside this range are removed, so the check fails closed.
...
mod tests {
    ...
    fn blocked_ranges() {
        ...
        let blocked_v6 = [
            ...
            "2001:2::",
            "2001:10::", // inside 2001::/23 (blocked whole, defect AX)
        ];
        ...
        // Review round 3, defect AX: the round 2 (defect AV) globally
        // reachable exceptions are removed. Each former exception now
        // reads as not public, the same as the rest of its enclosing
        // block.
        assert!(
            !is_public(IpAddr::V4("192.0.0.9".parse().unwrap())),
            "former 192.0.0.9/32 exception, now blocked"
        );
        assert!(
            !is_public(IpAddr::V4("192.0.0.10".parse().unwrap())),
            "former 192.0.0.10/32 exception, now blocked"
        );
        assert!(
            !is_public(IpAddr::V6("::ffff:192.0.0.9".parse().unwrap())),
            "mapped former 192.0.0.9/32 exception, now blocked"
        );
        assert!(
            !is_public(IpAddr::V6("2001:1::1".parse().unwrap())),
            "former 2001:1::1/128 exception, now blocked"
        );
        assert!(
            !is_public(IpAddr::V6("2001:1::2".parse().unwrap())),
            "former 2001:1::2/128 exception, now blocked"
        );
        assert!(
            !is_public(IpAddr::V6("2001:1::3".parse().unwrap())),
            "former 2001:1::3/128 exception, now blocked"
        );
        assert!(
            !is_public(IpAddr::V6("2001:3::".parse().unwrap())),
            "former 2001:3::/32 exception, now blocked"
        );
        assert!(
            !is_public(IpAddr::V6("2001:4:112::".parse().unwrap())),
            "former 2001:4:112::/48 exception, now blocked"
        );
        assert!(
            !is_public(IpAddr::V6("2001:20::".parse().unwrap())),
            "former 2001:20::/28 exception, now blocked"
        );
        assert!(
            !is_public(IpAddr::V6("2001:30::".parse().unwrap())),
            "former 2001:30::/28 exception, now blocked"
        );
        assert!(
            !is_public(IpAddr::V6("2001:10::".parse().unwrap())),
            "2001:10::/28, inside blocked 2001::/23"
        );
