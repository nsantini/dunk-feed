...
- **PRD phase**: 3
- **Size**: standard
- **Design**: docs/01-TECH-DESIGN.md §9
...
down, and drops pairs carrying a label in `UPSTAGE_DROP_LABELS`. Before the
...
- Does not change the block or detach handling already in
  `scorer/verify.rs`.
- Does not retune `UPSTAGE_FOLLOWER_FLOOR` itself; this story only measures
  and logs.
...
logic but only logs its would-be drops for the first `UPSTAGE_GUARD_LOG_ONLY_H`
...
| `src/config.rs` | Adds `UPSTAGE_GUARD_LOG_ONLY_H` (new, see Defaults taken). `UPSTAGE_DROP_LABELS` exists from story 01 |

## Behaviour contracts

| Id | Subject | Case | Behaviour |
|---|---|---|---|
| BC1 | follower floor | `O`'s author `followers < UPSTAGE_FOLLOWER_FLOOR` | Drop, reason `follower_floor` |
| BC2 | follower floor | `UPSTAGE_FOLLOWER_FLOOR = 0` | Guard disabled; no pair drops on follower count |
...
| BC8 | labels | any label on `Q`, `O`, or either author's profile matches `UPSTAGE_DROP_LABELS` | Drop, reason `labelled` |
| BC9 | labels | label present but not in `UPSTAGE_DROP_LABELS` | Pair unaffected |
...
| BC11 | log-only window | within `UPSTAGE_GUARD_LOG_ONLY_H` hours of the first scorer pass | Guard decisions are logged with the pair's follower count and the reason it would have used; no pair is dropped by the follower floor |
...
- [ ] AC3 — Labels in `UPSTAGE_DROP_LABELS` drop pairs; others pass. Checked by: `cargo test scorer::guards::tests::labels`
...
- `UPSTAGE_DROP_LABELS` comes from story 01's config parser. This story only reads it.
- `UPSTAGE_GUARD_LOG_ONLY_H` is likewise new, not in §4. Default 24, per this
...
window (`UPSTAGE_GUARD_HISTOGRAM_H`, was `UPSTAGE_GUARD_LOG_ONLY_H`) only controls
