//! `GET /healthz` (BC14, BC15, BC36, BC42): reports whether the service
//! still keeps up with Jetstream and with the scorer, so a container
//! healthcheck can restart it when it does not. Reads only `HealthState`'s
//! two atomics and the snapshot's length — no store read on the serving
//! path.
...
use crate::http::{AppState, HttpConfig};
use crate::store;

#[derive(Debug, Serialize, PartialEq)]
pub struct HealthBody {
    /// `None` (serialised as JSON `null`) until the ingest loop has recorded
    /// its first Jetstream commit (BC41, BC42).
    pub jetstream_lag_s: Option<i64>,
    /// `None` (serialised as JSON `null`) until the scorer has recorded its
    /// first successful pass (BC41, BC42).
    pub last_pass_age_s: Option<i64>,
...
/// exactly instead of racing the wall clock (BC14, BC15). An unobserved
/// clock (`None`) is unhealthy (BC42), the same as a clock that has drifted
/// past `cfg.health_max_lag_s`.
fn body_and_status(
    health: &HealthState,
    cfg: &HttpConfig,
    snapshot_len: usize,
    ...
    let healthy = matches!(jetstream_lag_s, Some(lag) if lag <= max)
        && matches!(last_pass_age_s, Some(age) if age <= max);
...
mod tests {
    ...
    async fn returns_200_within_thresholds() {
        ...
        state.health.set_commit_time(store::unix_now());
        state.health.set_scorer_pass(store::unix_now());
    ...
    // BC41, BC42: neither clock has ever been observed. 503, and both ages
    // serialise as JSON `null`, but `snapshot_len` is still a plain number.
    #[tokio::test]
    async fn unhealthy_until_first_commit_and_pass() {
        let cfg = test_config("127.0.0.1:0");
        let state = test_state(cfg);
        let app = router(state);
        ...
        let body = axum::body::to_bytes(response.into_body(), usize::MAX).await.unwrap();
        let json: Value = serde_json::from_slice(&body).unwrap();
        assert!(json["jetstream_lag_s"].is_null());
        assert!(json["last_pass_age_s"].is_null());
        assert_eq!(json["snapshot_len"], 0);
    }
    ...
    #[test]
    fn flips_at_300s() {
        let cfg = HttpConfig::from(&test_config("127.0.0.1:0"));
    ...
    fn body_shape_is_the_same_on_503() {
        ...
        assert_eq!(body.jetstream_lag_s, Some(1_000));
        assert_eq!(body.last_pass_age_s, Some(1_000));
